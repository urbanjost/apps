...
character(len=:),allocatable  :: version_text(:)
   ...
   if(size(filenames).ge.3)then
      ...
      write(*,'(a)')'<ERROR> insufficient arguments: replace- OLD NEW [OPTIONS] FILE(S)'
   endif
...
logical                      :: exists, open
   ...
   if(ios.ne.0)then
      ...
      write(stderr,*)'*replace-* ERROR : '//msg
      exit OKBLOCK
   endif
...
'   (LICENSE:MIT)                                            ',&
'SYNOPSIS                                                    ',&
'    replace- [ --ignorecase][ --dryrun][ --verbose] old new filename(s)',&
'                                                                       ',&
'     or                                                                ',&
'                                                                       ',&
...
'   to make a copy of the original before converting it. replace-(1) prints',&
