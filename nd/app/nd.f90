! (Regression testing)
!  nd(1): Detect numeric changes in output files
!
!  A simple Fortran program for comparing two files for numeric differences.
!  nd(1) is typically used in regression testing to detect changes in numeric results in ported or recompiled routines.
!===================================================================================================================================
!()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()(
!===================================================================================================================================
program numdiff
...
!-----------------------------------------------------------------------------------------------------------------------------------
   if (filein_old  ==  ' ' .or. filein_new  ==  ' ') then             ! if filenames are not specified stop
      print *, "*nd* error: old and new filenames are required."      ! report missing filename
      stop 10
   endif
!-----------------------------------------------------------------------------------------------------------------------------------
   if (idigits  /=  0) then                                   ! check for inappropriate number of digits to test that is too small
      significant_digits = int(log10(2.0**digits(0.0d0)))     ! MAXIMUM NUMBER OF SIGNIFICANT DIGITS IN A DOUBLE NUMBER.
      if (idigits  <  0) then                                 ! check for inappropriate number of digits to test that is too small
         write (warning, '(a,i0)') '*nd* bad number of significant digits=', idigits
         call remark(warning)
         idigits = significant_digits                         ! set bad value to maximum allowed for double precision number
      else if (idigits  >  significant_digits) then           ! check for inappropriate number of digits to test  that is too big
         write (warning, '(a,i0)') '*nd* significant digit requested too high=', idigits
         call remark(warning)
         idigits = significant_digits                         ! set bad value to maximum allowed for double precision number
      endif
   endif
!-----------------------------------------------------------------------------------------------------------------------------------
   write (*, '(132("-"))')
   print *, "       | *nd* numerical differences"                               ! display command options
   ...
   write (*, '(132("-"))')
   if (GLOBAL_COUNT  /=  0) then    ! return command exit value to system where supported
      STOP 1
   endif
contains
!===================================================================================================================================
...
!-----------------------------------------------------------------------------------------------------------------------------------
   ...
   if (ios  /=  0) then
      call remark('*nd* error: could not open file specified with -old')
      stop 20
   endif
!-----------------------------------------------------------------------------------------------------------------------------------
   open (unit=22, file=file_new(:len_trim(file_new)), status="old", iostat=ios)            ! open file of new values
   if (ios  /=  0) then
      call remark('*nd* error: could not open file specified with -new')
      stop 30
   endif
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
   INFINITE: do
      ...
      if (lline  /=  rline) then                                           ! if lines are not equal parse them and check differences
         if (idigits  /=  0) then
         ...
         elseif (margin  /=  0) then
            ...
            if (per  >=  percent_tolerance) then                          ! print lines over percentage tolerance
               write (*, '("--------|",a)') repeat('-', 132 - 9)
               write (*, "('old     |',a)") trim(lline(:ilen))
   ...
   if (ic  /=  0) then                                       ! ic=number of strings found
      do i = 1, ic
         ...
         if (iflag  ==  0) then                              ! value = real value of string
            ...
            if (iflag2  /=  0) then                              ! line1 was number but line2 was not
               ier = -1
         ...
         elseif (line(is(i):ie(i))  /=  line2(is2(i):ie2(i))) then  ! non-numeric stings
      ...
      do i = 1, ic
         ...
         if (iflag  ==  0) then                              ! value = real value of string
            ...
            if (iflag2  /=  0) then                              ! line1 was number but line2 was not
               ...
               if (ind  /=  0) then  ! if found a difference over limit, fill dline with # characters
                  GLOBAL_COUNT = GLOBAL_COUNT + 1
                  do ii = is(i), ie(i)
                     dline(ii:ii) = '#'
                  enddo
      ...
      do i = 1, ic
         ...
         if (iflag  ==  0) then                                ! value = real value of string
            ...
            if (iflag2  /=  0) then                            ! line1 was number but line2 was not
               ...
               if (diff*100.0  >=  percent_tolerance) then  ! if found a difference over limit, fill dline with # characters
                  GLOBAL_COUNT = GLOBAL_COUNT + 1
                  do ii = is(i), ie(i)
                     dline(ii:ii) = '#'
                  enddo
...
!-----------------------------------------------------------------------------------------------------------------------------------
   if (ilen  ==  0) return                                            ! command was totally blank
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
      if(digi <= 0)then                                        ! check for inappropriate number of digits to test that is too small
      ...
      else if(digi  >  ireal_significant_digits)then          ! check for inappropriate number of digits to test  that is too big
...
!-----------------------------------------------------------------------------------------------------------------------------------
      if (diff  ==  0.0) then                                  ! no difference
         acurcy = digi
      else if (y  ==  0.0) then                                ! special case where Y is zero
