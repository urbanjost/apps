...
use M_unicode, only : isascii, slurp, repeat, pound_to_box
...
'   translating UTF-8 to ASCII-7 with C-style escape sequences, changing         ',&
'   case, ...                                                                    ',&
'   (LICENSE:PD)                                                                 ',&
'                                                                                ',&
'SYNOPSIS                                                                        ',&
'    uni [--escape|--noescape] [--lcase|--ucase] | --code | --boxes STYLE |      ',&
'    [--start|--finish] | --wide | --length infile(s)                            ',&
'                                                                                ',&
'DESCRIPTION                                                                     ',&
'   uni(1) is a handy filter for converting UTF-8 encoded text to and            ',&
'   from ASCII-7 and C-style escape sequences, converting the case of            ',&
...
'                                                                                ',&
'   Note the largest valid Unicode codepoint is U+10FFFF (or 1,114,111 in        ',&
'   decimal). This limit was established to ensure compatibility with the        ',&
'   UTF-16 encoding, which uses surrogate pairs to represent characters          ',&
'   beyond the Basic Multilingual Plane (BMP). Key details about the             ',&
'   Unicode codespace:                                                           ',&
'                                                                                ',&
...
'                                                                                ',&
'   Encoding Limits: The limit U+10FFFF was intentionally set because it         ',&
'   is the highest value that can be represented using a single or a pair        ',&
'   of 16-bit code units in the UTF-16 encoding.                                 ',&
'                                                                                ',&
'   UTF-8: While the UTF-8 encoding scheme is theoretically capable              ',&
'   of representing much larger codepoints (up to 0x7FFFFFFF), it was            ',&
'   restricted by RFC 3629 to stop at U+10FFFF to match the Unicode              ',&
...
'                                                                                ',&
'   --boxes,B     box style choice from set {"light","bold","double"}.           ',&
...
character(len=:),allocatable :: iomsg
   ...
   if(iomsg.ne.'')then
      ...
      write(*,'(*(a:))',advance='no')(textout(i)%character(),new_line('a'),i=1,size(textout))
   endif
