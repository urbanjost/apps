...
!$SYSTEM numdiff -old ../tmp/_delim1.txt -new ../tmp/_delim2.txt
!$SYSTEM rm -f ../tmp/_delim1.txt ../tmp/_delim2.txt
!$!----------------------------------------------------------------------------------------------------------------------------------
!
!        starting tests of numdiff
!
!        Initialize database entry for test
!$SYSTEM goodbad numdiff start
!
!        Run the numdiff(1) program on the test files
!$SYSTEM numdiff -old ../tmp/_in1 -new ../tmp/_in2 -percent 0.00001 >../tmp/_test1.txt
!
!        Use diff(1) on expected results and results and update database accordingly
!$SYSTEM diff -b ../tmp/_expect1.txt ../tmp/_test1.txt && goodbad numdiff good
!
!        Clean up when tests complete successfully
!$SYSTEM rm ../tmp/_expect1.txt ../tmp/_in1 ../tmp/_in2 ../tmp/_test1.txt
!
!------------------------------------------------------------------------------------------------------------------------------------
!         numdiff UNIT TEST COMPLETE
!------------------------------------------------------------------------------------------------------------------------------------
!#export GITHUB=FALSE
!#export DEMO_OUTDIR=../../example
!#export DEMO_SUBDIR=FALSE
!export UFPP_DOCUMENT_DIR=$(pwd)
!ufpp TESTPRG90 F90 --cstyle doxygen --verbose --allow_links -i numdiff.[fF][fF] -o ../numdiff.f90
!txt2man doc/numdiff.1.man >../../man/man3/numdiff.1
!man2html ../../man/man3/numdiff.1 > ../../docs/numdiff.1.html
!gzip -f ../../man/man3/numdiff.1
!(
!cd ../..
!)
!exit
