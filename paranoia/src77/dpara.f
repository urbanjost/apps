...
C
     ...
     ~                   HALF, MINUS1
        DOUBLE PRECISION FP0, FP1, FP2, FP3, FP4, FP8, FP9, FP27, FP32,
     ~                   HALF, MINUS1
C
...
C
C
C DISABLE INTERRUPTS FOR LAHEY F77L.
C       LOGICAL FLAG
C       CALL INVALOP(FLAG)
C       CALL OVEFL(FLAG)
C       CALL UNDFL(FLAG)
C       CALL DVCHK(FLAG)
C
C DISABLE INTERRUPTS FOR MICROSOFT FORTRAN  (MASK MUST BE INTEGER*2)
C       INTEGER*2 MASK
C       MASK=4927
C       CALL LCWRQQ(MASK)
C DISABLE INTERRUPTS FOR WATFIV
...
6241    FORMAT (' except possibly for Double Rounding during Gradual',
     ~         ' Underflow.')
6250    WRITE(OUT, 6251)
...
C
1690    X = DABS( ((FP4 / FP3 - FP1) - FP1 / FP4) * FP3 - FP1 / FP4)
1700    Z2 = X
        X = (FP1 + (HALF * Z2 + FP32 * Z2 * Z2)) - FP1
        IF (Z2 .GT. X .AND. X .GT. FP0) GOTO 1700
1710    Y = DABS( (FP3/FP4 - FP2/FP3) * FP3 - FP1/FP4)
        Z=Y
        X=Y
1720    Z1=Z
...
C        ... SQUARE OF SQUARE ROOT OF Z (WRONG)
        DOUBLE PRECISION Y
C        ... DOUBLE PRECISION NUMBER WHOSE SQUARE ROOT SQUARED IS WRONG
        COMMON /STDIO/ IN, OUT
        INTEGER IN, OUT
...
1410    RADIX=DINT(.01 + ULPPLS/ULPMIN)
...
1580    X=-T8*DLOG(ULPMIN)/DLOG(RADIX)
        Y=DINT(HALF+X)
...
C       PURIFY INTEGERS.
1600    IF (PRECIS .EQ. DINT(PRECIS)) GOTO 1640
1610    WRITE(OUT,1611)
...
980     MINONE=-1.0E0
     ...
     ~ MINONE+DABS(MINONE) .EQ. 0.0E0 .AND.
...
C       ... LOCAL VARIABLES
        DOUBLE PRECISION D, D4, E5, E6, E7, Q, U, X, X1, X8,
     ~                   Y, Y1, Y2, Z, Z1, Z2
...
3400    E5 = E5 + HALF
        E7 = E7 - HALF
3410    Y = (DSQRT(FP1 + ULPPLS) - FP1)/ULPPLS
        E6 = (Y - FP1) + ULPPLS/FP8
        IF (E6 .GT. E7) E7 = E6
3420    E6 = Y + ULPPLS/FP8
        IF (E6 .LT. E5) E5 = E6
3430    Y = ((DSQRT(ONEMIN) - ULPPLS) - (FP1 - ULPPLS))/ULPMIN
        E6 = Y + ULPMIN/FP8
        IF (E6 .GT. E7) E7 = E6
...
3480    IF (I .EQ. 4) GOTO 3530
        IF (I .EQ. 2) GOTO 3520
3500    X = U * DSIGN(FP1,X) * DINT( FP8 / (FP9 * DSQRT(U)) )
        GOTO 3460
C
3520    U = ULPMIN
        X = -U
...
3560    D = DINT(HALF + RADIX ** (FP1 + PRECIS - DINT(PRECIS)))
C
...
3830    Z2 = D
        I = 0
C               COUNT HOW MANY TESTS OF DSQRT(D*X) = Y YIELD RESULTS.
3840    Y = D + (FP1 + Z) * HALF
        X = D + Z + Q
...
3951    FORMAT(' SERIOUS DEFECT: sqrt gets too many last digits wrong.')
        ...
        END
C---
C       ____ SUBROUTINE TO ASSESS ERROR  DSQRT(X*X) - X  IN ULPS. ____
C
...
C
        ...
        B1 = 1.0 / RADIX
3090    E6 = ((DSQRT(X * X) - X * B1) - (X - X * B1)) / U
        IF (E6 .EQ. 0.0) RETURN
...
C               DSQRT(D*X) = DSQRT((Y - 1/2)^2 + X8/2) ROUNDS TO  Y
C
...
5140    IF (.NOT. (Q .EQ. UFLTHR .AND. MINDIF .EQ. MINPOS .AND.
     ~   DABS(UFLTHR-MINDIF/NULPS) .LE. MINDIF)) GO TO 5010
        WRITE(OUT,5150)
        WRITE(OUT,5155)
...
5220    MILES=130
        CALL PAGE(MILES)
C       ---- PAUSE ---- ==================================
5230    Y = -DINT(HALF - 240.0 * DLOG(UFLTHR) / DLOG(H1)) / 240
        Y2=Y+Y
        WRITE(OUT,5240)H1,Y
